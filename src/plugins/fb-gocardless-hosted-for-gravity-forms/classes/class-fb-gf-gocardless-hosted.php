...
if ( method_exists( 'GFForms', 'include_payment_addon_framework' ) ) {
	...
	class Fb_Gf_Gocardless_Hosted extends GFPaymentAddOn {
		...
		public function success_redirect_to_gocardless_confirmation_url() {
			...
			if ( $entry && $form ) {
				...
				if ( $response && is_object( $response ) && isset( $response->api_response->status_code ) && 200 === (int) $response->api_response->status_code ) {
					...
					do_action( 'gf_gocardless_hosted_before_success_redirect', $entry, $form );

					// Send notifications.
					$notifications = $this->get_notifications_to_send( $form );
					if ( $notifications ) {
		 ...
		 *
		 * @return array An array of notification ids.
		 */
		public function get_notifications_to_send( $form ) {
