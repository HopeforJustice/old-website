<?php
/**
 * The template used for /unlock with UTM tracking 
 *
...
<?php
	...
	if($userInfo && in_array($userInfo, $us_donate)){
	    $url .= '/donate/us/?utm_source=IIAD&utm_medium=Book&utm_campaign=IIADunlock';
	} elseif ($userInfo && in_array($userInfo, $no_donate)) {
		$url .= '/donate/no/?utm_source=IIAD&utm_medium=Book&utm_campaign=IIADunlock';
	} else {
		$url .= '/donate/uk/?utm_source=IIAD&utm_medium=Book&utm_campaign=IIADunlock';
	}
	//echo $url;
	wp_redirect($url);
	exit;
